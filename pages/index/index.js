...
Page({
  ...
  bindViewTapPlay: function(e) {
    ...
    var id = parseInt(e.currentTarget.id) //0-52
    console.log(id,'id')
    var lycode = e.target.dataset.key //ee
    var lyindex = e.target.dataset.lyindex //601
    var title = e.target.dataset.title //拥抱每一天
    ...
    if(this.data['lylist_data'][id].hasOwnProperty('url')){
  ...
  onLoad: function () {
    ...
    //目录分类
    this.setData({
        lylist_categorys : wx.getStorageSync('category'),
    })
    //所有节目
    this.setData({
        lylist_data : wx.getStorageSync('lylist_data')
    })
